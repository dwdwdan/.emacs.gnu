...
**** Agenda
Here I'm assuming that I will have my laptop with me at any point.
#+begin_src emacs-lisp
...
I also need to turn of ~display-line-numbers-mode~ in the agenda, so tags appear on the right, rather than on the next line:
#+begin_src emacs-lisp
...
**** Refile
We want to be able to refile from ~inbox.org~ to ~gtd.org~ and ~reference.org~
#+begin_src emacs-lisp
