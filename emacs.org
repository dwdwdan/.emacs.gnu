...
#+begin_src emacs-lisp
  ...
  I use a tiling WM, so ediff creating new frames is really annoying. Therefore we can disable it here.
  #+begin_src emacs-lisp
  (use-package ediff
    :custom
...
To display icons nicely, we also need the ~all-the-icons~ package.
#+begin_src emacs-lisp
